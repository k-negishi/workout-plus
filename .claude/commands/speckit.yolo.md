...
YOLO! You Only Live Once! 仕様策定から実装までを一気通貫で実行することで、迅速にプロトタイピングし、フィードバックループを回すためのコマンドです。
...
- **実装前に「実装済み確認」を行う**: 対象コンポーネント・テストファイルを読み、Issue の要件がすでにコードで満たされていないか確認する。
  - 実装済みと判定した場合: 新規コードを書かず、品質チェックのみ実施してIssueに「実装済み確認」コメントを投稿してクローズする。
  - 未実装と確認できた場合: 通常の TDD（Red → Green）フローで実装する。
