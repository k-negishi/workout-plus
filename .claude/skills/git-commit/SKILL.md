...
> **⚠️ 他エージェントの作業中変更が混入するリスク**
>
> マルチエージェント環境では、自分が触っていないファイルが `git status` に現れることがある。
> 差分の内容が正当でも、**今回のタスクで自分が編集したファイルかどうか**を必ず確認する。
>
> **チェック方法**: 会話コンテキスト内で編集・作成したファイルの一覧と `git status` を照合する。
> 一致しないファイルは**コミットに含めない**。
>
> ```bash
> # 疑わしいファイルはステージしない。ワーキングツリーにそのまま残す。
> git add <自分が触ったファイルのみ明示列挙>
> ```
>
> **失敗パターン（実例）**: 別エージェントが作業中の `migrations.ts` が未コミットのまま残っており、
> 差分確認後「正当な変更だ」と判断してコミットしてしまった。
> 内容の正しさと「今回のタスクか」は別問題。
...
- **他エージェントの作業中ファイルをコミットに含める**（差分が正当でも今回のタスクでなければ含めない）

## 実行前チェックリスト

- 差分を確認した
- **`git status` の全ファイルが今回の会話で自分が編集したものか確認した**
- prefixの根拠を説明できる
