...
# 基本方針
- 特に指示がない場合、サブエージェントを5~10つ立ち上げてください。
- 指示がない限りは sonnet 4.6 を使ってください
- ソースコードには日本語でコメントを書くこと

# 実装ルール（必須）
- **機能追加・バグ修正は必ず `implementer` エージェント（Agent ツール, subagent_type: implementer）に委譲すること**
- **メインエージェント（私自身）がプロダクションコードを直接書いてはならない**
- 実装完了後は必ず `reviewer` エージェントを起動してレビューを行うこと
