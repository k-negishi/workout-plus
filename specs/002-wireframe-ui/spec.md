...
**対象範囲**: MVP スコープとして定義した以下の 7 画面 + タブバー
- ホーム画面
- 記録画面（ワークアウトセッション）
...
2. **Given** ホーム画面を表示する、**When** StreakCard を確認する、**Then** 今月のトレーニング日数（数値大きく）+ 炎アイコン + 直近 7 日の小円インジケーターが表示される
...
- **FR-007**: SafeAreaView または useSafeAreaInsets を使用して動的に安全領域を取得すること

**ホーム画面**

- **FR-008**: StreakCard は今月のトレーニング日数（大きな数値）と直近 7 日間の各日完了/休息インジケーターを表示しなければならない
...
| StrealCard | 実装済み | ワイヤーフレームの 7 日インジケーター形式と要確認 |
